...
.eni-messages{flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:12px;margin-bottom:16px;padding-right:4px;max-height:400px}
...
.eni-toast{position:fixed;bottom:90px;right:30px;background:rgba(0,0,0,0.9);backdrop-filter:blur(10px);padding:10px 18px;border-radius:12px;font-size:13px;font-weight:500;z-index:10000000;animation:slideUp 0.3s ease;pointer-events:none}
...
${ST.view==='chat'?`
<div class="eni-badge">📁 ${ST.p?ST.p.slice(0,8)+'...':'Projeto'}</div>
<div class="eni-messages" id="eni-msgs"></div>
<div class="eni-input-area">
<textarea id="eni-in" placeholder="Descreva sua alteração..." rows="2"></textarea>
<div style="display:flex;justify-content:flex-end">
<button class="eni-send-btn" id="eni-send-btn" onclick="window.enS()">Enviar →</button>
</div>
</div>
`:`
<div class="eni-badge">🏢 ${ST.w.slice(0,8)}...</div>
...
<button class="eni-create-btn" id="eni-create-btn" onclick="window.enC()">🚀 Criar Projeto</button>
</div>
`}
`}</div>`;const c=document.getElementById('eni-msgs');if(c){c.innerHTML='';ST.msgs.forEach(m=>{const g=document.createElement('div');g.className=`eni-bubble ${m.r==='u'?'eni-user':'eni-bot'}`;g.textContent=m.t;c.appendChild(g)});c.scrollTop=c.scrollHeight}}
...
window.enS=async()=>{const i=document.getElementById('eni-in');const btn=document.getElementById('eni-send-btn');if(!i||!ST.p)return;const msg=i.value.trim();if(!msg)return;ST.msgs.push({r:'u',t:msg});i.value='';ui();btn.disabled=true;btn.innerHTML='<span class="eni-loading"><span class="eni-spinner"></span> Enviando</span>';const result=await sendMessageToLovable(ST.t,ST.p,msg);btn.disabled=false;btn.innerHTML='Enviar →';if(result.success&&result.status===202){showToast(result.message,'success');ST.msgs.push({r:'bt',t:'✅ Mensagem processada com sucesso!'});ui();setTimeout(()=>{const idx=ST.msgs.findIndex(m=>m.t==='✅ Mensagem processada com sucesso!');if(idx!==-1){ST.msgs.splice(idx,1);ui()}},2500)}else{showToast(result.message,'error');ST.msgs.push({r:'bt',t:result.message});ui();setTimeout(()=>{const idx=ST.msgs.findIndex(m=>m.t===result.message);if(idx!==-1){ST.msgs.splice(idx,1);ui()}},3000)}};
window.enC=async()=>{const d=document.getElementById('eni-new-desc');const btn=document.getElementById('eni-create-btn');if(!d||!d.value.trim())return;const desc=d.value.trim();btn.disabled=true;btn.innerHTML='<span class="eni-loading"><span class="eni-spinner"></span> Criando...</span>';const result=await createNewProject(ST.t,ST.w,desc);if(result.success){showToast('✅ Projeto criado! Redirecionando...','success');setTimeout(()=>{window.location.href=result.link},800)}else{showToast(result.message,'error');btn.disabled=false;btn.innerHTML='🚀 Criar Projeto'}};
